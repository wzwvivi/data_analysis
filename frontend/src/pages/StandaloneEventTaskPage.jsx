 ...
 */
function StandaloneEventTaskPage() {
  ...
  const loadTaskDetail = useCallback(async (id) => {
    ...
    try {
      const taskRes = await standaloneEventApi.getTask(id)
      setAnalysisTask(taskRes.data)
      if (taskRes.data.status === 'completed') {
        const [resultsRes, timelineRes] = await Promise.all([
          standaloneEventApi.getCheckResults(id),
          standaloneEventApi.getTimeline(id),
        ])
  ...
  useEffect(() => {
    ...
    if (polling && analysisTask?.status === 'processing' && taskId) {
      timer = setInterval(async () => {
        try {
          const res = await standaloneEventApi.getTask(taskId)
  ...
  const handleExport = async () => {
    ...
    try {
      setExporting(true)
      const res = await standaloneEventApi.exportResults(taskId)
  ...
  const handleViewDetail = async (record) => {
    ...
    try {
      const res = await standaloneEventApi.getCheckDetail(taskId, record.id)
...
export default StandaloneEventTaskPage
