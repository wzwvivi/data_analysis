# -*- coding: utf-8 -*-
"""飞管事件分析路由（原 event_analysis，Phase 1 renamed）"""
import io
import uuid
...
from ..background_jobs import (
    run_fms_event_analysis_task_job,
    run_standalone_fms_event_analysis_task_job,
)
from ..task_executor import submit_process_job


router = APIRouter(
    prefix="/api/fms-event-analysis",
    tags=["飞管事件分析"],
...
async def run_event_analysis(
    ...
    - `bundle_version_id`（可选，MR4）：指定本次分析使用的 Bundle 版本。
      省略时默认跟随 ParseTask.protocol_version_id。前端"按其他版本重跑"功能
      会带上此参数。
    """
    service = FmsEventAnalysisService(db)

    try:
        task = await service.get_or_create_analysis_task(
            parse_task_id, rule_template, bundle_version_id=bundle_version_id,
        )
    except FmsEventAnalysisService.BundleVersionMismatch as exc:
        # 该任务已锁定到某个 bundle 版本，不允许按其他版本跑
        raise HTTPException(
            status_code=409,
            detail={
                "code": "BUNDLE_VERSION_LOCKED",
                "message": str(exc),
                "existing_version_id": exc.existing_version_id,
                "requested_version_id": exc.requested_version_id,
            },
        )
...
def _task_to_response(task, bundle_version_label: Optional[str] = None) -> EventAnalysisTaskResponse:
...
async def _resolve_bundle_version_label(db: AsyncSession, version_id: Optional[int]) -> Optional[str]:
    """按 ProtocolVersion.id 查 version 字符串，用于前端展示 'v3.2.1'（不存在返回 None）。"""
    if not version_id:
        return None
...
async def upload_standalone_pcap(
    ...
    - `bundle_version_id`（可选，MR4）：指定本次分析锁定的 TSN 协议版本。
      未传则默认不锁定，服务会取当前任意可用 bundle（向后兼容）。
    """
    ...
    background_tasks.add_task(
        submit_process_job, run_standalone_fms_event_analysis_task_job, task.id
    )
...
async def standalone_from_shared_pcap(
    ...
    """使用平台共享 TSN 文件创建独立事件分析（直接读取共享源文件，不复制）。"""
...
async def list_standalone_tasks(
    ...
    # 批量查 ProtocolVersion.version 映射，避免 N+1
    vids = [int(t.bundle_version_id) for t in items if t.bundle_version_id]
    label_map: dict = {}
    if vids:
        from sqlalchemy import select as _select
        from ..models import ProtocolVersion
        res = await db.execute(
            ...
            .where(ProtocolVersion.id.in_(set(vids)))
        )
