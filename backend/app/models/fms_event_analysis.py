# -*- coding: utf-8 -*-
"""
飞管事件分析相关数据模型（Phase 1b 拆表后独立于飞控事件分析）。

表对应关系（Phase 1b）：
- `fms_event_analysis_tasks`  ← 原 `event_analysis_tasks`（`rule_template ∈ {default_v1, NULL}`）
- `fms_event_check_results`   ← 原 `event_check_results`
- `fms_event_timeline_events` ← 原 `event_timeline_events`

旧类名 `EventAnalysisTask` / `EventCheckResult` / `EventTimelineEvent` 继续以
别名形式保留，下一个大版本再删。
"""
...
class FmsEventAnalysisTask(Base):
    """飞管事件分析任务。"""
    __tablename__ = "fms_event_analysis_tasks"
    ...
    bundle_version_id = Column(
        ...
        comment="本次分析使用的 Bundle 版本（即 ProtocolVersion.id，MR4 版本锁定）",
    )
    ...
    parse_task = relationship("ParseTask", backref="fms_event_analysis_tasks")
    check_results = relationship("FmsEventCheckResult", back_populates="analysis_task", cascade="all, delete-orphan")
    timeline_events = relationship("FmsEventTimelineEvent", back_populates="analysis_task", cascade="all, delete-orphan")


class FmsEventCheckResult(Base):
    """飞管事件分析检查单结果。"""
    __tablename__ = "fms_event_check_results"

    id = Column(Integer, primary_key=True, index=True)
    analysis_task_id = Column(Integer, ForeignKey("fms_event_analysis_tasks.id"), nullable=False)
    ...
    analysis_task = relationship("FmsEventAnalysisTask", back_populates="check_results")


class FmsEventTimelineEvent(Base):
    """飞管事件分析时间线事件。"""
    __tablename__ = "fms_event_timeline_events"
    ...
    related_check_id = Column(Integer, ForeignKey("fms_event_check_results.id"), nullable=True)
    ...
    analysis_task = relationship("FmsEventAnalysisTask", back_populates="timeline_events")
    related_check = relationship("FmsEventCheckResult", backref="related_events")


# ── Phase 1 向后兼容别名（可在 Phase 1b 之后逐步移除） ──
