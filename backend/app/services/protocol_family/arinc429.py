...
spec_json 结构（移植自桌面 generator + parser 硬编码规律提炼）：
{
    ...
    # ──────────────────────────────────────────────
    # 顶层 port_routing（P1）：承载 parser 里硬编码的 _PORT_LABELS
    # 键为 UDP 端口号字符串，值为该端口监听的 label_oct 列表
    # 与其让 parser 代码里硬编码"ADC 的 7001 端口只收 1/3/5 号 label"，
    # 不如放进 bundle，这样每版协议自己声明自己的端口路由
    # ──────────────────────────────────────────────
    "port_routing": {
        "7001": ["001", "003", "005"],
        "8032": ["312", "362", "001", "005", "011"]
    },
...
def _normalize_port_routing(value: Any) -> Dict[str, List[str]]:
    """顶层 port_routing：{port: [label_oct, ...]}

    约定：label 统一为 3 位零填充八进制字符串（与 labels[].label_oct 对齐）。
    - int 输入按十进制解释，转成 oct
    - str 输入按八进制直接 zfill
    """
    if not isinstance(value, dict):
    ...
    for port, labels in value.items():
        ...
        for lab in labels:
            if isinstance(lab, int):
                ...
                s = str(lab).strip().zfill(3) if str(lab).strip() else ""
            if not s:
                continue
            if s not in seen:
                seen.add(s)
                norm_labels.append(s)
        out[str(port)] = norm_labels
    return out
...
class Arinc429FamilyHandler:
    ...
    def normalize_spec(self, spec_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        spec["port_routing"] = _normalize_port_routing(spec.get("port_routing"))
        return spec
    ...
    def validate_spec(self, spec_json: Dict[str, Any]) -> SpecValidation:
        ...
        # 顶层 port_routing 校验
        port_routing = spec.get("port_routing") or {}
        if port_routing and not isinstance(port_routing, dict):
            out.errors.append("port_routing 必须是对象")
        else:
            declared_labels = seen_octs
            for port, labs in port_routing.items():
                ppath = f"port_routing[{port}]"
                if not isinstance(labs, list):
                    out.errors.append(f"{ppath} 必须是数组")
                    continue
                try:
                    p = int(port)
                    if not (0 < p < 65536):
                        out.warnings.append(
                            f"{ppath}: port={port} 超出 UDP 端口范围"
                        )
                except (TypeError, ValueError):
                    out.warnings.append(f"{ppath}: port 应为整数字符串")
                for lab_oct in labs:
                    if str(lab_oct) not in declared_labels:
                        out.warnings.append(
                            f"{ppath}: label={lab_oct} 未在 labels[] 中声明"
                        )
        return out
    ...
    def diff_spec(
        ...
        if old:
            ...
            # 顶层 port_routing 变化也进 meta_changed
            old_routing = old_norm.get("port_routing") or {}
            new_routing = new_norm.get("port_routing") or {}
            if old_routing != new_routing:
                out.meta_changed["port_routing"] = {
                    "old": old_routing,
                    "new": new_routing,
                }
        else:
    ...
    def summarize_spec(self, spec_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "port_routing_port_count": len(port_routing),
        }
