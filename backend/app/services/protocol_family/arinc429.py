...
spec_json 结构（移植自桌面 generator + parser 硬编码规律提炼）：
{
    ...
    # 注：port_routing（UDP 端口 → labels）归属 TSN 网络协议
...
class Arinc429FamilyHandler:
    ...
    def normalize_spec(self, spec_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # port_routing 归属 TSN 网络协议，静默丢弃遗留字段（若存在）
    ...
    def validate_spec(self, spec_json: Dict[str, Any]) -> SpecValidation:
        ...
        # 注：port_routing 归属 TSN 网络协议（BundlePort.arinc_labels），
        # 此处不再参与设备 ICD 的校验。
    ...
    def diff_spec(
        ...
        if old:
            ...
            # 注：port_routing 归 TSN 网络协议，不参与设备 ICD 的 diff
        else:
            # 全量新增
